...
const useGameStore = create<GameState>((set) => ({
  ...
  markVisited: (id) => set((state) => {
    ...
    state.showToast(`Found: ${loc.title}`);
    return {
      locations: newLocs,
...
const MiniMapUI = () => {
  ...
  return (
    <div id="minimap" className="absolute bottom-6 right-6 w-40 h-40 rounded-full border-4 border-[#d4a373] bg-black/80 overflow-hidden shadow-2xl pointer-events-auto">
...
const UI = () => {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10 font-sans">
      ...
      {/* --- INTRO SCREEN --- */}
      {showIntro && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/90 pointer-events-auto z-[100] backdrop-blur-md"
...
const ToastNotification = () => {
  ...
  return (
    <div className="absolute top-33 right-6 z-50 pointer-events-none">
      <div id="Notification" >{toastMessage}</div>
    </div>
  );
};

...
const SceneManager = ({ playerRef }: { playerRef: React.RefObject<THREE.Group | null> }) => {
  ...
  const { scene: templeModel } = useGLTF(`${basePath}/models/ShriKhadokali.glb`);
  ...
  return (
    <>
      ...
      {locations.map(loc => (
        <RigidBody type="fixed" colliders="trimesh" key={loc.id}>
      ...
      >
        ...
        <Canvas shadows camera={{ fov: 45, position: [0, 40, 80] }}>
          <React.Suspense fallback={null}>
            {/* 1. Wrap everything that needs physics here */}
            <Physics debug={false}>
